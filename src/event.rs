...
use crate::{
    canvas::{input, palette, tools, Canvas},
    window,
};
                ...
                Event::Mouse(MouseEvent { kind, x, y }) => match kind {
                    ...
                    EventKind::Release(_) => {
                        if let Some(new_color) = palette::get_color(&clickable_colors, x, y) {
                            ...
                            canvas
                                .terminal
                                .write(&format!("{:02},{:02},{:?}             ", x, y, new_color));
                            color = new_color;
                            canvas.terminal.flush();
                        }
                    ...
                    EventKind::Move if palette.is_some() => {
                        if let Some(new_color) = palette::get_color(&clickable_colors, x, y) {
                            window::draw(&mut canvas.terminal, palette::WIDTH, new_color);
                        } else {
                            window::draw(&mut canvas.terminal, palette::WIDTH, color);
                        }
                        canvas.terminal.flush();
                        last_point = None;
                    }
                ...
                Event::Key(key) => match key {
                    ...
                    KeyEvent::Char(char, _) if palette.is_some() => {
                        ...
                        if let Some(input_field) = &mut input_field {
                            if input_field.input().len() != palette::WIDTH as usize {
